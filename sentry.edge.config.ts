...
// pollers). Excluding them from tracesSampler avoids the per-invocation cost
// of building + shipping a span to Sentry on every hit, which otherwise adds
// real Active CPU time on Vercel Fluid Compute for endpoints that are polled
// far more often than actual users hit the app.
